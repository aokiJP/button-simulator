...
system.runInterval(() => {
    for (const player of world.getAllPlayers()) {
        ...
        if (multiplayer)
            player.setDynamicProperty(properties.money, cash + multiplayer * magnifications.money);
        else
            player.setDynamicProperty(properties.money, cash + 1 * magnifications.money);
        if (rebirths)
